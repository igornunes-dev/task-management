...
interface InProgressTasksProps extends PageProps {
    tasks: PaginatedTasks;
}

const StatusBadge = ({ status }: { status: 'pending' | 'in_progress' | 'completed' }) => {
    const statusConfig = {
        pending: {
...
export default function InProgress({ tasks }: InProgressTasksProps) {
    const breadcrumbs: BreadcrumbItem[] = [
        { title: 'Dashboard', href: dashboard().url },
        { title: 'Tarefas em Andamento', href: taskrouter.in_progress().url },
    ];

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Tarefas em Andamento" />

            <div className="flex h-full flex-1 flex-col gap-8 p-4 md:p-8">
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-3xl font-bold text-white">Tarefas em Andamento</h1>
                        <p className="text-gray-400">Acompanhe as tarefas que estão sendo executadas.</p>
                    </div>
                    <Link
                    ...
                    <div className="divide-y divide-[#3E3E3A]">
                        {tasks.data.length > 0 ? (
                            ...
                            <div className="flex h-48 items-center justify-center">
                                <p className="text-gray-400">Nenhuma tarefa em andamento encontrada!</p>
                            </div>
                        )}
                    </div>
                </div>
