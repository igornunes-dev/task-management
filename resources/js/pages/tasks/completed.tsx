...
interface CompletedTasksProps extends PageProps {
...
export default function Completed({ tasks }: CompletedTasksProps) {
    const breadcrumbs: BreadcrumbItem[] = [
        { title: 'Dashboard', href: dashboard().url },
        { title: 'Tarefas Concluídas', href: taskrouter.completed().url },
    ];
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-8 p-4 md:p-8 text-gray-900 dark:text-white">
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-3xl font-bold">Tarefas Concluídas</h1>
                        <p className="text-gray-500 dark:text-gray-400">Veja todas as tarefas que já foram finalizadas.</p>
                    </div>
                    <Link
                    ...
                    <div className="divide-y divide-gray-200 dark:divide-[#3E3E3A]">
                        {tasks.data.length > 0 ? (
                                        ...
                                        <div className="flex items-center gap-4">
                                            <div>
                                                ...
                                                <p className="text-xs text-gray-500 dark:text-gray-400">
                                                    Concluída em: {new Date(task.updated_at).toLocaleDateString()}
                                                </p>
                                            </div>
                                        </div>
                            ...
                            <div className="flex h-48 items-center justify-center">
                                <p className="text-gray-500 dark:text-gray-400">Nenhuma tarefa concluída encontrada!</p>
                            </div>
                        )}
                    </div>
                </div>
